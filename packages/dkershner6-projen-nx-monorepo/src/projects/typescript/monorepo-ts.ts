...
import { Dependency, DependencyType, Project, Task, YamlFile } from "projen";
    ...
    implements INxProjectCore
{
    ...
    constructor(options: MonorepoTsProjectOptions) {
        ...
        switch (this.package.packageManager) {
            ...
            case NodePackageManager.NPM: {
                // Allow older versions of peer deps to resolve compatibility issues
                this.tasks
                    .tryFind("install")
                    ?.reset("npm install --legacy-peer-deps");
                this.tasks
                    .tryFind("install:ci")
                    ?.reset("npm ci --legacy-peer-deps");
                break;
            }
        }
        ...
        if (
            ...
            options.scripts["post-compile"] == null
        ) {
    ...
    protected linkLocalWorkspaceBins(): void {
        ...
        this.subprojects.forEach((subProject) => {
            if (
                ...
                const pkgBins: Record<string, string> =
                    (subProject as NodeProject).package.manifest.bin() || {};
                bins.push(
        ...
        (this.tasks.tryFind("prepare") || this.addTask("prepare")).spawn(
            linkTask,
        );
    }
     ...
     */
    private resolveSubNodeProjects(): void {
        if (this.subNodeProjectResolves.length) {
            ...
            const completedResolves = this.subNodeProjectResolves.map(
                (resolve) => resolve(),
            );
     ...
     */
    private validateSubProjects(): void {
        this.subprojects.forEach((subProject: any) => {
            ...
            if (
                NodePackageUtils.isNodeProject(subProject) &&
                subProject.package.packageManager !==
                    this.package.packageManager
            ) {
                throw new Error(
    ...
    private updateWorkspace(): void {
        ...
        // Automatically add all sub-project "bundledDependencies" to workspace "nohoist", otherwise they are not bundled in npm package
        if (
     ...
     * @private
     */
    private installNonNodeDependencies(): void {
     ...
     */
    private disableNodeWarnings(): void {
                    ...
                    .forEach((d) => {
                        switch (d.type) {
                            case DependencyType.RUNTIME:
                                (subProject as NodeProject).addDeps(
                                    `${d.name}@workspace:*`,
                                );
                                break;
                            case DependencyType.BUILD:
                                (subProject as NodeProject).addDevDeps(
                                    `${d.name}@workspace:*`,
                                );
                                break;
                            case DependencyType.PEER:
                                (subProject as NodeProject).addPeerDeps(
                                    `${d.name}@workspace:*`,
                                );
                                break;
                            default:
...
function getBundledDeps(project: Project): Dependency[] {
    return project.deps.all.filter(
        (dep) => dep.type === DependencyType.BUNDLED,
    );
}
